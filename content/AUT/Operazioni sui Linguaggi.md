...
- **Concatenazione**: Se abbiamo (per le stringhe)$$ \begin{gather} x = a_1,...,a_n \ , \quad y=b_1,...,b_m \quad n,m \gt 0 \\ xy = a_1,...,a_n,b_1,...,b_m \in \Sigma^* \\ \epsilon x = x \epsilon = x \\ \begin{cases} x \epsilon = x \\ x(ya)=(xy)a \\ x,y \in \Sigma ^*, \ a \in \Sigma \end{cases} \end{gather}$$ Posso concatenare anche linguaggi:  $L_1 \circ L_2 = \{xy \ : \ x \in L_1 \ e \ y \in L_2$ .
...
- **Potenza**: Per le stringhe: $$\begin{gather} x^n=x...x \quad n \ volte \quad x \in \Sigma^* \\ x^0=\epsilon \\ x^{u+1}=x^ux \end{gather}$$
Lo stesso vale per i linguaggi $$\begin{gather} L^0 = \{\epsilon\} \\ L^{u+1}=L^u \circ L \end{gather}$$
