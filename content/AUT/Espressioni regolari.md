...
> Sviluppando: $$\begin{align} (0 \cup 1) &= \{ 0, \ 1\} \\ 0^* &= \{0\}^* \\ (0 \cup 1) 0^* &= \{0, \ 1\} \circ \{0\}^* \end{align}$$

## Definizione Formale

Sia $\Sigma$ un alfabeto. Espressione regolare su $\Sigma$, denominata $re(\Sigma)$ è definita ricorsivamente:

**Caso base**: $$\begin{cases} \emptyset \in re(\Sigma) \\ \epsilon \in re(\Sigma) \\ a \in re(\Sigma) , \ a \in \Sigma \end{cases}$$**Caso induttivo**: $$\begin{cases} R_1 \cup R_2 \quad &R_1,R_2 \in re(\Sigma) \\ R_1 \circ R_2 \quad &R_1,R_2 \in re(\Sigma) \\ (R_1)^* \quad &R_1 \in re(\Sigma) \end{cases}$$
